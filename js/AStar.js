...
function aStarAlgorithm(grid, startPos, endPos){
...
function aStarIteration(grid, openSet,  endPos){
    ...
    if (openSet.length > 0){
        ...
        neighbourPositions = getValidNeighbourPositions(currBest, grid)
        for(let i = 0; i < neighbourPositions.length; i++){
            neighbourPos = neighbourPositions[i]
            ...
            if(grid[neighbourPos.x][neighbourPos.y] == OPEN){
                ...
                neighbour = new Node(neighbourPos, currBest, g, h, f)
